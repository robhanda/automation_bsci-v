##############################################################################################################################
# file name   : pre_memory_check.py
# date        : 20-08-2017
# author      : Pavan Kumar
# version     : 0.1
# description : To check memory check of PQ and DSP before the test case (i.e. Pre memory readings)
		#a> First reading With sites are UP but GENA is N
		#b> Second reading is GENA is Y but dring that time ELG macro are not running  
#################################################################################################################################
#!/usr/bin/env python
...
#from /home/Automation/parameters 
import input_file
def telnetFunction():
	global telnetBsc
...
def memleakCommandafterGenaDisable():
	if input_file.bsc_type == "Flexi":
		...
		output_file1 = open('/home/Automation/memoryleak/pq_mem_pre1.txt','w+')
		pcuVersion="ssv"
...
def memleakCommandafterGenaEnable():
	if input_file.bsc_type == "Flexi":
		telnetBsc.read_until(b'expectedBscLine',10)
		cmd=("ZDDS:BCSU,%s,,PCU2,%s:;"%(input_file.bcsu_id,input_file.pcu_id))
		telnetBsc.write(cmd+'\n\r')
		telnetBsc.read_until(b'OSE>',10)
		output_file1 = open('/home/Automation/memoryleak/pq_mem_pre2.txt','w+')
		pcuVersion="ssv"
		telnetBsc.write(pcuVersion+'\r')
		outputpcuVersion=telnetBsc.read_until(b'OSE>',10)
		print outputpcuVersion
...
if __name__ == "__main__":
	print("executing pre memory leak script")
	telnetFunction()
	genaDisable()
	dis_commandfiring()
	time.sleep(120)
